...
class JiaziCrawler:
    ...
    def _parse_jiazi_data(
        self, data: Dict[str, Any]
    ) -> Tuple[List[Book], List[BookSnapshot]]:
        """解析夹子榜API响应数据"""
        books = []
        snapshots = []
    ...
    async def _save_data(
        self, books: List[Book], snapshots: List[BookSnapshot]
    ) -> Tuple[List[Book], List[BookSnapshot]]:
        """保存数据到数据库"""
        saved_books = []
        saved_snapshots = []
        ...
        logger.info(
            f"数据保存完成: {len(saved_books)} 本书籍, {len(saved_snapshots)} 个快照"
        )
