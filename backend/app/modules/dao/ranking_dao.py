...
class RankingDAO(BaseDAO):
    ...
    def get_ranking_history_summary(
        self, ranking_id: str, days: int = 7
    ...
    def batch_create_snapshots(
        self, snapshots_data: List[Dict[str, Any]]
    ...
    def get_snapshots_by_date_range(
        self, ranking_id: str, start_date: datetime, end_date: Optional[datetime] = None
    ) -> List[RankingSnapshot]:
        """获取指定日期范围的榜单快照"""
        if end_date is None:
    ...
    def count_ranking_books(
        self, ranking_id: str, snapshot_time: Optional[datetime] = None
    ) -> int:
        """获取指定榜单在指定时间的书籍总数"""
        # 如果没有指定时间，使用最新快照时间
        if snapshot_time is None:
        ...
        statement = select(func.count(RankingSnapshot.id)).where(
            and_(
                RankingSnapshot.ranking_id == ranking_id,
                RankingSnapshot.snapshot_time == snapshot_time,
            )
