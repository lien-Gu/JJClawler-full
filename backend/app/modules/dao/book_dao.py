...
class BookDAO(BaseDAO):
    ...
    def search(
        ...
        if author:
            conditions.append(
                or_(Book.author_name.contains(author), Book.author_id == author)
            )
        if novel_class:
        ...
        # 分页查询
        statement = (
            statement.offset(offset).limit(limit).order_by(Book.last_updated.desc())
        )
    ...
    def get_snapshots_by_date_range(
        self, book_id: str, start_date: datetime, end_date: Optional[datetime] = None
    ) -> List[BookSnapshot]:
        """获取指定日期范围的快照"""
        if end_date is None:
