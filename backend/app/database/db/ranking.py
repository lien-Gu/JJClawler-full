...
from sqlalchemy import DateTime, Integer, String, Float, Index, UniqueConstraint
...
class Ranking(Base):
    ...
    rank_group_type: Mapped[Optional[str]] = mapped_column(
        String(50), 
        nullable=True,
...
class RankingSnapshot(Base):
    ...
    ranking_id: Mapped[int] = mapped_column(
        Integer, 
        index=True,
        comment="关联的榜单ID，对应Ranking表的id字段"
    )
    
    novel_id: Mapped[int] = mapped_column(
        Integer, 
        index=True,
        comment="关联的书籍ID，对应Book表的novel_id字段"
    )
    ...
    position: Mapped[int] = mapped_column(
        Integer, 
        index=True,
        comment="书籍在榜单中的排名位置，数字越小排名越高，从1开始"
    )
    
    score: Mapped[Optional[float]] = mapped_column(
        Float, 
        nullable=True,
        comment="排名分数，某些榜单会提供具体的评分数据，可选字段"
    )
