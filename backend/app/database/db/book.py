...
class BookSnapshot(Base):
    ...
    recommendations: Mapped[int] = mapped_column(Integer, default=0, comment="推荐数量，读者推荐该书籍的总数")

    # 内容信息  
    word_count: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, comment="字数统计，书籍当前的总字数")
    status: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, comment="书籍状态，如：连载中、已完结等")
