...
class HttpClient:
    ...
    async def run_concurrently(self, urls: List[str]) -> List[Dict]:
        ...
        tasks = [self._request_and_get_content_async(self.async_client, url, semaphore) for url in urls]
    ...
    def run_synchronously(self, urls: List[str]) -> List[Dict]:
        ...
        results = []
        for url in urls:
            result = self._request_and_get_content_sync(self.client, url, self.timeout)
    ...
    @staticmethod
    def _request_and_get_content_sync(
