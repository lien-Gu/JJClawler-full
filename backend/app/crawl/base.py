...
class CrawlConfig:
    ...
    def build_url(self, task_config: Dict) -> str:
        ...
        template_url = self._config["global"]["templates"].get(template_name)
        
        if not template_url:
            raise ValueError(f"模板不存在: {template_name}")
        
        # 合并全局参数和任务参数
        params = {**self._config["global"]["base_params"], **task_config["params"]}
...
class BaseCrawler:
    ...
    def __init__(self, parser: BaseParser, rate_limit: float = 1.0):
        """
        初始化爬虫
        ...
        self.client = HttpClient(rate_limit=rate_limit)
        self.stats = {
            'total_requests': 0,
