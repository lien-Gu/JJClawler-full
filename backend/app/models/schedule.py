...
class JobResultModel(BaseModel):
    ...
    @classmethod
    def success_result(
        cls, message: str, data: dict[str, Any] = None
    ) -> "JobResultModel":
        """创建成功结果"""
    ...
    @classmethod
    def error_result(
        cls, message: str, exception: Exception = None
    ) -> "JobResultModel":
        """创建失败结果"""
        return cls(
            success=False,
...
class JobConfigModel(BaseModel):
    ...
    interval_seconds: int | None = Field(
        None, ge=60, le=86400, description="间隔秒数"
    )
    cron_expression: str | None = Field(None, description="Cron表达式")
    run_date: datetime | None = Field(None, description="指定执行时间")

    # 任务数据
    page_ids: list[str] = Field(
        default_factory=list, description="页面id列表，可以是特殊字符all/jiazi/category"
    )
    batch_id: str | None = Field(None, description="批量任务ID，用于跟踪相关任务")
    parent_task_id: str | None = Field(
        None, description="父任务ID，用于任务依赖关系"
    )

    @property
    def is_single_page_task(self) -> bool:
        """判断是否为单页面任务"""
        return len(self.page_ids) == 1 and self.page_ids[0] not in [
            "all",
            "jiazi",
            "category",
        ]
