...
class HTTPClient:
    ...
    def __init__(
        ...
        self.rate_limit_delay = (
            rate_limit_delay if rate_limit_delay is not None else settings.CRAWL_DELAY
        )
...
async def create_http_client(
    timeout: float = 30.0, rate_limit_delay: float = 1.0, max_retries: int = 3
) -> HTTPClient:
    """
    创建HTTP客户端的便捷函数
    ...
    return HTTPClient(
        timeout=timeout, rate_limit_delay=rate_limit_delay, retry_config=retry_config
    )
