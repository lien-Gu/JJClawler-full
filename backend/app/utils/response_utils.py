...
def success_response(data: Any = None, message: str = "操作成功") -> ApiResponse:
    """构建成功响应"""
    return ApiResponse(
        success=True, code=StatusCode.SUCCESS, message=message, data=data
    )


def error_response(code: StatusCode, message: str) -> ApiResponse:
    """构建错误响应"""
    return ApiResponse(success=False, code=code, message=message, data=None)


def paginated_response(
    data: list, page: int, page_size: int, total_count: int, message: str = "查询成功"
) -> ApiResponse:
    """构建分页响应 - 分页信息嵌套在data中"""
