...
def book_to_detail(
    book: Book, latest_snapshot: Optional[BookSnapshot] = None
) -> BookDetail:
    """
...
def calculate_position_change(
    current_position: Optional[int], previous_position: Optional[int]
) -> int:
    """
    计算位置变化
...
def format_ranking_item(
    book: Book, snapshot: BookSnapshot, position: int, position_change: int = 0
) -> Dict[str, Any]:
    """
    格式化排行榜项目
