...
async def get_book_detail(book_id: str = Path(..., description="书籍ID")):
    """
    获取书籍详细信息
        ...
        with service_context(BookService) as book_service:
            ...
            if not book_detail:
                error_resp = error_response(
                    code=StatusCode.BOOK_NOT_FOUND, message="书籍不存在"
                )
                raise HTTPException(status_code=404, detail=error_resp.model_dump())
            return success_response(data=book_detail, message="获取书籍详情成功")
...
async def get_book_rankings(
    ...
    try:
        with service_context(BookService) as book_service:
            book_detail, current_rankings, history_rankings = (
                book_service.get_book_ranking_history(book_id, days)
            )
