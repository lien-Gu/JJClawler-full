...
async def get_rankings_list(
    ...
    try:
        ...
        for ranking_info in paged_rankings:
            ...
            last_updated = None
            try:
                _, books, snapshot_time = ranking_service.get_ranking_books(
                    ranking_info.ranking_id, limit=1
                )
        ...
        error_resp = error_response(
            code=StatusCode.INTERNAL_ERROR, message="获取榜单列表失败"
        )
    ...
    try:
        ...
        for ranking_info in all_rankings:
            ...
            try:
                ...
                # 获取最近7天的趋势数据来计算活跃度
                trend_data = ranking_service.get_ranking_trend_data(
                    ranking_info.ranking_id, days=7
                )
                recent_activity = sum(
                    item.get("snapshot_count", 0) for item in trend_data
                )
        ...
        return success_response(
            data={"rankings": top_hot_rankings, "total": len(top_hot_rankings)},
            message="获取热门榜单成功",
        )

    except Exception as e:
        error_resp = error_response(
            code=StatusCode.INTERNAL_ERROR, message="获取热门榜单失败"
        )
        raise HTTPException(status_code=500, detail=error_resp.model_dump())
    finally:
        ranking_service.close()
...
async def search_rankings(
    ...
    try:
        ...
        error_resp = error_response(
            code=StatusCode.INTERNAL_ERROR, message="搜索榜单失败"
        )
        raise HTTPException(status_code=500, detail=error_resp.model_dump())
...
async def get_ranking_books(
    ...
    try:
        ...
        # 获取榜单书籍数据
        ranking_info, books_in_ranking, actual_snapshot_time = (
            ranking_service.get_ranking_books(ranking_id, snapshot_time, limit, offset)
        )

        if not ranking_info:
            error_resp = error_response(
                code=StatusCode.RANKING_NOT_FOUND, message="榜单不存在"
            )
            raise HTTPException(status_code=404, detail=error_resp.model_dump())
...
async def get_ranking_history(
    ...
    """
    try:
        ranking_info, summaries = ranking_service.get_ranking_history_summary(
            ranking_id, days
        )
