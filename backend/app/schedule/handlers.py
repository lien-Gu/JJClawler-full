...
class BaseJobHandler(ABC):
    ...
    @abstractmethod
    async def execute(self, context: JobContextModel) -> JobResultModel:
        """执行任务（子类必须实现）"""
        pass
    
    async def execute_with_retry(self, context: JobContextModel) -> JobResultModel:
        """带重试机制的任务执行"""
    ...
    async def on_success(self, context: JobContextModel, result: JobResultModel) -> None:
        """任务执行成功时的回调"""
        self.logger.debug(f"任务 {context.job_id} 执行成功: {result.message}")
    
    async def on_failure(self, context: JobContextModel, exception: Optional[Exception]) -> None:
        """任务执行失败时的回调"""
        self.logger.error(f"任务 {context.job_id} 执行失败: {exception}")
    
    async def on_retry(self, context: JobContextModel, attempt: int) -> None:
        """任务重试时的回调"""
        ...
        try:
            ...
            try:
                if job_type == 'jiazi':
                    ...
                    return JobResultModel.success_result(f"夹子榜爬取完成", {"crawled_count": crawled_count, "task_id": task_id})
                    ...
                    return JobResultModel.success_result(f"分类榜单爬取完成", {"crawled_count": crawled_count, "task_id": task_id})
                    
                elif job_type == 'pages':
                    # 执行指定页面爬取任务
                    page_ids = context.job_data.get('page_ids', [])
                    if not page_ids:
                        return JobResultModel.error_result("页面ID列表为空")
                    ...
                    return JobResultModel.success_result(f"指定页面爬取完成", {"crawled_count": crawled_count, "page_ids": page_ids, "task_id": task_id})
                    
                else:
                    await self._update_task_status(task_id, "failed", f"未知的爬虫任务类型: {job_type}")
                    return JobResultModel.error_result(f"未知的爬虫任务类型: {job_type}")
            ...
            return JobResultModel.error_result(f"爬虫任务执行失败: {str(e)}", e)
    ...
    async def execute(self, context: JobContextModel) -> JobResultModel:
        ...
        try:
            ...
            if job_type == 'database_cleanup':
                ...
                return JobResultModel.success_result("数据库清理完成", {"cleaned_records": 1000})
                
            elif job_type == 'log_rotation':
                # 执行日志轮转
                await asyncio.sleep(0.5)  # 模拟任务执行
                return JobResultModel.success_result("日志轮转完成", {"rotated_files": 5})
                
            elif job_type == 'health_check':
                # 执行健康检查
                await asyncio.sleep(0.3)  # 模拟任务执行
                return JobResultModel.success_result("系统健康检查完成", {"status": "healthy"})
                
            else:
                return JobResultModel.error_result(f"未知的维护任务类型: {job_type}")
                
        except Exception as e:
            return JobResultModel.error_result(f"维护任务执行失败: {str(e)}", e)
    ...
    async def execute(self, context: JobContextModel) -> JobResultModel:
        ...
        try:
            ...
            if job_type == 'data_analysis':
                ...
                return JobResultModel.success_result("数据分析报告生成完成", {"report_size": "2.5MB"})
                
            else:
                return JobResultModel.error_result(f"未知的报告任务类型: {job_type}")
                
        except Exception as e:
            return JobResultModel.error_result(f"报告任务执行失败: {str(e)}", e)