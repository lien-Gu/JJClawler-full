...
class TestGetBookDetail:
    ...
    def test_get_book_detail_success(self, client, mocker, mock_book_data, mock_book_snapshot_data):
        """测试成功获取书籍详情"""
        # 模拟service返回数据
        mock_service = mocker.patch('app.api.books.book_service')
        mock_service.get_book_detail_with_latest_snapshot.return_value = {
            "book": type('MockBook', (), mock_book_data["book"])(),
            "latest_snapshot": type('MockSnapshot', (), mock_book_snapshot_data["latest_snapshot"])()
        }
...
class TestGetBookTrend:
    ...
    def test_get_book_trend_success(self, client, mocker, mock_book_data, mock_book_snapshot_data):
        """测试成功获取书籍趋势"""
        # 模拟service
        mock_service = mocker.patch('app.api.books.book_service')
        mock_service.get_book_by_id.return_value = type('MockBook', (), mock_book_data["book"])()
        ...
        # 发送请求
        response = client.get("/api/v1/books/1/trend?days=7")
        
        # 验证响应
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        assert "获取7天趋势数据成功" in data["message"]
        assert data["count"] == 2
        assert len(data["data"]) == 2
    ...
    def test_get_book_trend_book_not_found(self, client, mocker):
        ...
        # 发送请求
        response = client.get("/api/v1/books/999/trend?days=7")
...
class TestGetBookTrendAggregated:
    ...
    def test_get_book_trend_hourly_success(self, client, mocker, mock_book_data, mock_book_snapshot_data):
        """测试成功获取小时级趋势"""
        # 模拟service
        mock_service = mocker.patch('app.api.books.book_service')
        mock_service.get_book_by_id.return_value = type('MockBook', (), mock_book_data["book"])()
        mock_service.get_book_trend_hourly.return_value = mock_book_snapshot_data["aggregated_data"]
    ...
    def test_get_book_trend_aggregated_success(self, client, mocker, mock_book_data, mock_book_snapshot_data):
        """测试通用聚合趋势接口"""
        # 模拟service
        mock_service = mocker.patch('app.api.books.book_service')
        mock_service.get_book_by_id.return_value = type('MockBook', (), mock_book_data["book"])()
        mock_service.get_book_trend_with_interval.return_value = mock_book_snapshot_data["aggregated_data"]
        
        # 发送请求
        response = client.get("/api/v1/books/1/trend/aggregated?period_count=7&interval=day")
    ...
    def test_get_book_trend_aggregated_invalid_interval(self, client):
        """测试无效的时间间隔参数"""
        response = client.get("/api/v1/books/1/trend/aggregated?period_count=7&interval=invalid")
        assert response.status_code == 422  # 参数验证失败
...
class TestGetBookRankingHistory:
    ...
    def test_get_book_ranking_history_success(self, client, mocker, mock_book_data, mock_ranking_history_data):
        """测试成功获取排名历史"""
        # 模拟service
        mock_book_service = mocker.patch('app.api.books.book_service')
        mock_book_service.get_book_by_id.return_value = type('MockBook', (), mock_book_data["book"])()
        
        mock_ranking_service = mocker.patch('app.api.books.ranking_service')
        mock_ranking_service.get_book_ranking_history.return_value = mock_ranking_history_data
    ...
    def test_get_book_ranking_history_no_ranking_id(self, client, mocker, mock_book_data, mock_ranking_history_data):
        """测试不指定榜单ID获取排名历史"""
        # 模拟service
        mock_book_service = mocker.patch('app.api.books.book_service')
        mock_book_service.get_book_by_id.return_value = type('MockBook', (), mock_book_data["book"])()
