...
class TestAPIDBIntegration:
    ...
    def test_books_api_with_real_database(self, client, test_db, sample_book_data):
        ...
        db.add(test_book)
        db.commit()  # 先提交book以获取ID
        
        # 创建书籍快照记录
        test_snapshot = BookSnapshot(
            novel_id=test_book.id,  # 使用实际的book.id作为外键
            favorites=sample_book_data.get("favorites", 1000),
            clicks=sample_book_data.get("total_clicks", 5000),
            comments=sample_book_data.get("total_comments", 100),
            recommendations=sample_book_data.get("total_recs", 50),
        ...
        # assert found_book["author"] == sample_book_data["author"]  # author字段已删除
    ...
    def test_book_detail_api_with_database(self, client, test_db, sample_book_data):
        ...
        # assert book_detail["author"] == sample_book_data["author"]  # author字段已删除
    ...
    def test_rankings_api_with_database(self, client, test_db, sample_ranking_data):
        ...
        test_ranking = Ranking(
            rank_id=sample_ranking_data.get("rank_id", 1001),
            rank_name=sample_ranking_data.get("name", "测试榜单"),
            rank_group_type=sample_ranking_data.get("type", "romance"),
            page_id=sample_ranking_data.get("page_id", "test_page")
        )
        db.add(test_ranking)
        db.commit()
    ...
    def test_book_trend_api_with_database(self, client, test_db, sample_book_data):
        ...
        for i in range(7):
            snapshot = BookSnapshot(
                novel_id=test_book.id,  # 使用book的ID作为外键
                favorites=1000 + i * 10,
                clicks=5000 + i * 50,
                comments=200 + i * 2,
    ...
    def test_data_consistency_across_apis(self, client, test_db, sample_book_data, sample_ranking_data):
        ...
        # assert book_data["author"] == ranked_book["author"]  # author字段已删除
