...
class TestBookService:
    ...
    def test_get_book_by_novel_id(self, book_service, db_session, create_test_book):
        """测试根据novel_id获取书籍"""
        result = book_service.get_book_by_novel_id(
            db_session, create_test_book.novel_id
        )
    ...
    def test_create_book(self, book_service, db_session):
        """测试创建书籍"""
        book_data = {"novel_id": 99999, "title": "新建测试小说"}
    ...
    def test_create_or_update_book_create(self, book_service, db_session):
        """测试创建或更新书籍 - 创建新书籍"""
        book_data = {"novel_id": 88888, "title": "创建或更新测试小说"}
    ...
    def test_create_or_update_book_update(
        self, book_service, db_session, create_test_book
    ):
        """测试创建或更新书籍 - 更新现有书籍"""
        book_data = {"novel_id": create_test_book.novel_id, "title": "更新的书名"}
    ...
    def test_search_books_by_title(
        self, book_service, db_session, create_multiple_books
    ):
        """测试根据标题搜索书籍"""
    ...
    def test_get_books_with_pagination_filters(
        self, book_service, db_session, create_multiple_books
    ):
        """测试带过滤条件的分页获取书籍列表"""
        filters = {"title": "测试小说1"}
        result = book_service.get_books_with_pagination(
            db_session, page=1, size=10, filters=filters
        )
    ...
    def test_get_books_by_ids_with_invalid_id(
        self, book_service, db_session, create_multiple_books
    ):
        """测试根据包含无效ID的列表获取书籍"""
    ...
    def test_batch_create_book_snapshots(
        self, book_service, db_session, create_test_book
    ):
        """测试批量创建书籍快照"""
        snapshots_data = [
    ...
    def test_get_latest_snapshot_by_book_id(
        self, book_service, db_session, create_test_book_snapshot
    ):
        """测试获取书籍最新快照"""
        result = book_service.get_latest_snapshot_by_book_id(
            db_session, create_test_book_snapshot.book_id
        )

        assert result is not None
        assert result.book_id == create_test_book_snapshot.book_id

    def test_get_snapshots_by_book_id(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试获取书籍快照列表"""
        ...
        assert (
            len(result) >= 2
        )  # create_multiple_snapshots为create_test_book创建了多个快照
        assert all(snapshot.book_id == create_test_book.id for snapshot in result)

    def test_get_snapshots_by_book_id_with_time_range(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试在指定时间范围内获取书籍快照列表"""
        ...
        result = book_service.get_snapshots_by_book_id(
            db_session, create_test_book.id, start_time=start_time, end_time=end_time
        )

        assert len(result) >= 1
        assert all(
            start_time <= snapshot.snapshot_time <= end_time for snapshot in result
        )

    # ==================== 趋势分析测试 ====================

    def test_get_book_trend_by_interval_day(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按天获取书籍趋势数据"""
        ...
        result = book_service.get_book_trend_by_interval(
            db_session, create_test_book.id, start_time, end_time, "day"
        )
    ...
    def test_get_book_trend_by_interval_hour(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按小时获取书籍趋势数据"""
        start_time = datetime(2024, 1, 14, 0, 0, 0)
        ...
        result = book_service.get_book_trend_by_interval(
            db_session, create_test_book.id, start_time, end_time, "hour"
        )

        assert isinstance(result, list)
        assert len(result) >= 1

    def test_get_book_trend_by_interval_invalid_interval(
        self, book_service, db_session, create_test_book
    ):
        """测试使用无效时间间隔"""
        start_time = datetime(2024, 1, 1, 0, 0, 0)
    ...
    def test_get_book_detail_with_latest_snapshot(
        self, book_service, db_session, create_test_book_snapshot
    ):
        """测试获取书籍详情和最新快照数据"""
        result = book_service.get_book_detail_with_latest_snapshot(
            db_session, create_test_book_snapshot.book_id
        )
    ...
    def test_get_book_detail_with_latest_snapshot_no_book(
        self, book_service, db_session
    ):
        """测试获取不存在书籍的详情"""
        result = book_service.get_book_detail_with_latest_snapshot(db_session, 99999)
        assert result is None

    def test_get_book_trend_hourly(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按小时获取书籍趋势数据"""
        result = book_service.get_book_trend_hourly(
            db_session, create_test_book.id, hours=48
        )
    ...
    def test_get_book_trend_daily(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按天获取书籍趋势数据"""
        result = book_service.get_book_trend_daily(
            db_session, create_test_book.id, days=30
        )

        assert isinstance(result, list)

    def test_get_book_trend_weekly(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按周获取书籍趋势数据"""
        result = book_service.get_book_trend_weekly(
            db_session, create_test_book.id, weeks=8
        )

        assert isinstance(result, list)

    def test_get_book_trend_monthly(
        self, book_service, db_session, create_multiple_snapshots, create_test_book
    ):
        """测试按月获取书籍趋势数据"""
        result = book_service.get_book_trend_monthly(
            db_session, create_test_book.id, months=6
        )
    ...
    def test_get_book_statistics_no_snapshots(
        self, book_service, db_session, create_test_book
    ):
        """测试获取没有快照的书籍统计信息"""
    ...
    def test_cleanup_old_snapshots(
        ...
        deleted_count = book_service.cleanup_old_snapshots(
            db_session, create_test_book.id, keep_days=30, keep_count=100
        )
    ...
    def test_get_latest_snapshot_by_book_id_no_snapshots(
        self, book_service, db_session, create_test_book
    ):
        """测试获取没有快照的书籍的最新快照"""
        # create_test_book没有关联的快照
        result = book_service.get_latest_snapshot_by_book_id(
            db_session, create_test_book.id
        )
        assert result is None
