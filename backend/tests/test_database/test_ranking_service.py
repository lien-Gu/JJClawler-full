...
class TestRankingService:
    ...
    def test_get_ranking_by_rank_id(self, ranking_service, db_session, create_test_ranking):
        """测试根据rank_id获取榜单"""
        result = ranking_service.get_ranking_by_rank_id(db_session, create_test_ranking.rank_id)
    ...
    def test_update_ranking(self, ranking_service, db_session, create_test_ranking):
        ...
        update_data = {
            "name": "更新后的榜单名",
            "rank_group_type": "更新后的分类"
        }
        
        result = ranking_service.update_ranking(db_session, create_test_ranking, update_data)
    ...
    def test_get_rankings_by_page_id(self, ranking_service, db_session, create_test_ranking):
        """测试根据页面ID获取榜单列表"""
        result = ranking_service.get_rankings_by_page_id(db_session, create_test_ranking.page_id)
        
        assert len(result) >= 1
        assert any(r.id == create_test_ranking.id for r in result)
    
    def test_get_rankings_by_group_type(self, ranking_service, db_session, create_test_ranking):
        """测试根据分组类型获取榜单"""
        result = ranking_service.get_rankings_by_group_type(db_session, create_test_ranking.rank_group_type)
        
        assert len(result) >= 1
        assert any(r.id == create_test_ranking.id for r in result)
    
    def test_get_rankings_with_pagination(self, ranking_service, db_session, create_test_ranking):
        """测试分页获取榜单列表"""
        result = ranking_service.get_rankings_with_pagination(db_session, page=1, size=10)
    ...
    def test_create_ranking_snapshot(self, ranking_service, db_session, create_test_ranking, create_test_book):
        """测试创建榜单快照"""
        snapshot_data = {
    ...
    def test_batch_create_ranking_snapshots(self, ranking_service, db_session, create_test_ranking, create_test_book):
        """测试批量创建榜单快照"""
        snapshots_data = [
            {
                "ranking_id": create_test_ranking.id,
            ...
            },
            {
                ...
                "position": 2,
                "score": 90.0,
                "snapshot_time": datetime(2024, 1, 15, 13, 0, 0)  # 不同的时间避免约束冲突
            }
        ]
        
        result = ranking_service.batch_create_ranking_snapshots(db_session, snapshots_data)
        
        assert len(result) == 2
        assert result[0].position == 1
        assert result[1].position == 2
    
    def test_get_latest_snapshots_by_ranking_id(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取榜单最新快照"""
    ...
    def test_get_snapshots_by_ranking_and_date(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取指定日期的榜单快照"""
    ...
    def test_get_book_ranking_history(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取书籍排名历史"""
    ...
    def test_get_ranking_statistics(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取榜单统计信息"""
        result = ranking_service.get_ranking_statistics(db_session, create_test_ranking_snapshot.ranking_id)
    ...
    def test_get_ranking_trend(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取榜单变化趋势"""
    ...
    def test_get_ranking_detail(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取榜单详情"""
        result = ranking_service.get_ranking_detail(db_session, create_test_ranking_snapshot.ranking_id)
    ...
    def test_get_ranking_detail_with_target_date(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取指定日期的榜单详情"""
        target_date = create_test_ranking_snapshot.snapshot_time.date()
    ...
    def test_get_ranking_history(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试获取榜单历史趋势"""
    ...
    def test_get_book_ranking_history_with_details(self, ranking_service, db_session, create_test_ranking_snapshot):
    ...
    def test_compare_rankings(self, ranking_service, db_session, create_test_ranking, create_test_ranking_snapshot):
        """测试对比多个榜单"""
        # 创建第二个榜单
        ranking_data2 = {
            "rank_id": 2,
    ...
    def test_cleanup_old_ranking_snapshots(self, ranking_service, db_session, create_test_ranking_snapshot):
        """测试清理旧的榜单快照"""
        # 创建一些旧快照
        old_snapshot_data = {
    ...
    def test_create_or_update_ranking_without_rank_id(self, ranking_service, db_session):
        """测试创建或更新榜单时缺少rank_id"""
        ranking_data = {
            "name": "测试榜单",
            "page_id": "test"
