...
class RequestManager {
     ...
     * 获取排行榜报告数据
     */
    async getRankingReport(rankingId) {
        try {
            console.log(`正在请求排行榜报告: /reports/latest/${rankingId}`);
            const response = await this.get(`/reports/latest/${rankingId}`);
            ...
            if (response && response.success && response.data) {
                ...
                console.warn('排行榜报告API响应格式不正确，使用模拟数据');
                return await this.getMockRankingReportFallback(rankingId);
            }
        } catch (error) {
            console.error('加载排行榜报告失败，使用模拟数据:', error);
            return await this.getMockRankingReportFallback(rankingId);
        }
    }
     ...
     * 排行榜报告的模拟数据回退方案
     */
    async getMockRankingReportFallback(rankingId) {
        const mockResponse = await mockingManager.getMockDataByPath(`/reports/latest/${rankingId}`);
        return {
            success: true,
